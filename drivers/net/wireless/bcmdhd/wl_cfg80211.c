...
static const struct ieee80211_regdomain brcm_regdom = {
	...
	.reg_rules = {
		 ...
		 * rejects a VHT80 SoftAp; BCM4358 supports VHT80. */
	...
	if (cfg->p2p_supported && (wlif_type != -1)) {
		...
		if (timeout > 0 && !wl_get_p2p_status(cfg, IF_ADDING) && cfg->if_event_info.valid) {
			...
			if (err != BCME_OK) {
				...
				bcm_add_crash_reason(((dhd_pub_t *)(cfg->pub))->crash_reason,
					"p2p_ifdel failed, error %d, sent HANG event to %s\n",
					err, ndev->name);
...
void wl_reinit_event_handler(void)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
				...
				if (!dfs_radar_disabled) {
					...
					if (!err) {
						...
						if (channel & WL_CHAN_PASSIVE) {
							/* Honor "passive" only on real-DFS (radar) channels;
							 * firmware tags every 5GHz passive, blocking non-DFS AP. */
							if (channel & WL_CHAN_RADAR) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, 0))
								band_chan_arr[index].flags |=
									IEEE80211_CHAN_PASSIVE_SCAN;
