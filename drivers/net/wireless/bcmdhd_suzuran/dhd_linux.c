 ...
 *
 * $Id: dhd_linux.c 718508 2017-08-31 02:48:38Z $
 */
...
static int dhd_sar_callback(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	...
	ret = dhd_iovar(dhd, 0, "sar_enable", (char *)&sar_enable, sizeof(sar_enable),
		NULL, 0, TRUE);
	if (ret < 0)
...
#ifdef CONFIG_BCM_WLAN_RAMDUMP
		bcm_add_crash_reason(dhdp->crash_reason,
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (roam_trigger[0] == WL_AUTO_ROAM_TRIGGER) {
		ret = dhd_iovar(dhd, 0, "roam_env_detection", (char *)&roam_env_mode,
			sizeof(roam_env_mode), NULL, 0, TRUE);
		if (ret == BCME_OK)
...
#endif /* USE_WL_TXBF */
#ifdef DISABLE_TXBFR
	ret = dhd_iovar(dhd, 0, "txbf_bfr_cap", (char *)&txbf_bfr_cap, sizeof(txbf_bfr_cap),
		NULL, 0, TRUE);
	if (ret < 0) {
	...
	u8* src_mac, u8* dst_mac, u32 period_msec)
{
	 ...
	 */
	res = dhd_iovar(dhd_pub, 0, "mkeep_alive", &mkeep_alive_id, sizeof(mkeep_alive_id),
		pbuf, TEMP_BUF_SIZE, FALSE);
